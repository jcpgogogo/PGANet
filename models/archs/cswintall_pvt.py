...
sys.path.append('/home/zt/work/test/NAFNet-test1/basicsr/models/archs')
#from restormer_arch import Attention,TransformerBlock
#from vision_transformer import Attention
from cswin_pvt import LePEAttention 
...
class Block(nn.Module):
    def __init__(self, input_dim, output_dim, head_dim, window_size, drop_path, type='W', input_resolution=None):
        ...
        self.msa = nn.ModuleList([
                LePEAttention(
                    ...
                    split_size=256//input_resolution, num_heads=1024//input_resolution, dim_out=input_dim//2,
                    qk_scale=None, attn_drop=0.,  proj_drop=0.,sr_ratio=input_resolution//32)
                for i in range(2)])
    ...
    def forward(self, x):
        ...
        #print('qkv:',qkv.shape)
        img_ = self.ln1(x_)
        qkv_ = self.qkv(img_).reshape(B, -1, 3, C).permute(2, 0, 1, 3)
        #print('qkv_:',qkv_.shape)
        x1 = self.msa[0](qkv[:,:,:,:C//2],qkv_[:,:,:,:C//2])
        x2 = self.msa[1](qkv[:,:,:,C//2:],qkv_[:,:,:,C//2:])

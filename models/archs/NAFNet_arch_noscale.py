...
class NAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[],n_scales=4,
        ...
        self.unpoolings_per_scale = nn.ModuleList([
            nn.ModuleList([
                    nn.ConvTranspose2d(chan_unpool, chan_unpool, 3, stride=2, padding=1,output_padding = 1)
        ...
        self.concat_per_scale =nn.ModuleList([           
                nn.Conv2d(chanconcat*2, chanconcat, kernel_size=1, padding=0, stride=1, groups=1, bias=True)
        ...
        # self.conv_blocks_per_scale = nn.ModuleList([
        #     nn.ModuleList([
        #         NAFBlock_galerkin(chan,0 if i_scale > 1 else 1) for i in range(self.n_convs_per_scale[i_scale])
        #     ])
        #     for i_scale in range(self.n_scales)
        # ])
        self.conv_blocks_per_scale = nn.ModuleList([
            nn.ModuleList([
                NAFBlock(chan)  for i in range(self.n_convs_per_scale[i_scale])
            ])
            for i_scale in range(self.n_scales)
        ])
        ...
        self.intro = nn.Conv2d(in_channels=img_channel, out_channels=chan, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
        self.ending = nn.Conv2d(in_channels=chan, out_channels=img_channel, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)

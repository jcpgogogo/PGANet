...
class WMSA(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, head_dim, window_size, type):
        ...
        self.kln = LayerNorm((self.input_dim, 1, self.head_dim))
        self.vln = LayerNorm((self.input_dim, 1, self.head_dim))
        self.norm1 = LayerNorm2d(self.input_dim)
    ...
    def get_lepe(self, x,func,h_windows):
        ...
        return x
    def get_lepek(self, x,func,h_windows):
        x = rearrange(x, 'h b w p c -> b w p (h c)')
        ...
        return x
    def get_lepev(self, x,func,h_windows):
        x = rearrange(x, 'h b w p c -> b w p (h c)')
    ...
    def forward(self, x):
        ...
        k = self.get_lepek(k, self.get_v,h_windows)
        # v = self.get_lepev(v, self.get_v,h_windows)
        # k = self.kln(k)
        # v = self.vln(v)
        # q = self.qln(q, self.get_v,h_windows)
        ...
        if self.type != 'W':
            ...
            # sim = sim.masked_fill_(attn_mask, float("-100"))

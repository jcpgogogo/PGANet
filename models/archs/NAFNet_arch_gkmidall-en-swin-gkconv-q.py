...
from basicsr.models.archs.galerkin_conv import simple_attn
...
class NAFBlock_midgalerkin(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        super().__init__()
        dw_channel = c * DW_Expand
        self.conv1 = nn.Conv2d(in_channels=c, out_channels=dw_channel, kernel_size=1, padding=0, stride=1, groups=1, bias=True)
        ...
        self.mlp = nn.Sequential(
            nn.Linear(dw_channel//2, 4 * dw_channel//2),
            nn.GELU(),
            nn.Linear(4 * dw_channel//2, dw_channel//2),
        )
        self.sca1 = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channels=ffn_channel // 2, out_channels=ffn_channel // 2, kernel_size=1, padding=0, stride=1,
                      groups=1, bias=True),
        )
        # dim=c
    ...
    def forward(self, inp):
        ...
        # spatial_map = self.spatial_interaction(attened_x)
...
class NAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        for num in enc_blk_nums:
            self.encoders.append(
                nn.Sequential(NAFBlock_swin(chan),
                    *[NAFBlock_midgalerkin(chan) for _ in range(num-1)]
                )
            )
            self.downs.append(
        ...
        self.middle_blks = \
            nn.Sequential(
                *[NAFBlock_midgalerkin(chan) for _ in range(middle_blk_num)]
            )

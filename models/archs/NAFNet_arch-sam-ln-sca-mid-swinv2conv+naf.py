...
import sys
sys.path.append('/home/lhc/work/NAFNet')
from basicsr.models.archs.arch_util import LayerNorm2d
from basicsr.models.archs.local_arch import Local_Base


from basicsr.models.archs.shiftvit import ShiftViTBlock
#from shiftvitall import ShiftViTBlock as ShiftViTBlockall
from basicsr.models.archs.swintallv2_conv import ConvTransBlock
# from basicsr.models.archs.smt import Attention as smtattention
from basicsr.models.archs.smt import Block as smtblock
...
class NAFBlock_smt(nn.Module):
    def __init__(self, c,DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        ...
        #                                'W' , input_resolution)]
        self.ctb = [ConvTransBlock(dw_channel // 8, dw_channel // 8, self.head_dim, self.window_size, 0,
        ...
        ca_num_heads=[4, 4, 4,4,4]
        sa_num_heads=[-1, -1, 8,8,8]
        mlp_ratios=[4, 4, 4,4,4]
        ca_attentions=[1, 1,  1,1,1]
        blocki=c//64
        self.blocki=blocki
        self.smt=smtblock(
                dim=dw_channel//2, ca_num_heads=ca_num_heads[blocki], sa_num_heads=sa_num_heads[blocki], mlp_ratio=mlp_ratios[blocki], qkv_bias=False, qk_scale=None,
                use_layerscale=False, layerscale_value=1e-4, drop=0., attn_drop=0.,
                    drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                ca_attention=ca_attentions[blocki], expand_ratio=2)
        # self.smt = nn.Sequential(*self.smt)
        # SimpleGate
        self.sg = SimpleGate()
    ...
    def forward(self, inp):
        ...
        if self.blocki<1:
            B,C,H,W= x.shape
            x = Rearrange('b c h w -> b (h w) c', h = H, w = W)(x)
            x = self.smt(x, H, W)
...
class NAFBlock_swin(nn.Module):
    def __init__(self, c,DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        ...
        # self.ctb = [ConvTransBlock(dw_channel // 2, dw_channel // 2, self.head_dim, self.window_size, 0,
        #                                'W' , input_resolution)]
        self.ctb = [ConvTransBlock(dw_channel // 4, dw_channel // 4, self.head_dim, self.window_size, 0,
                                       'W' if not i % 2 else 'SW', input_resolution) for i in range(2)]
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[]):
        ...
        for num in enc_blk_nums:
            self.encoders.append(
                nn.Sequential(NAFBlock_smt(chan),
        ...
        for num in dec_blk_nums:
            ...
            self.decoders.append(
                nn.Sequential(
                    *[NAFBlock_smt(chan) for _ in range(num)]
                )
            )

...
class LePEAttention(nn.Module):
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=8, attn_drop=0., proj_drop=0., qk_scale=None,sr_ratio=1):
        super().__init__()
        self.dim = dim
        ...
        if idx == -1:
            ...
            H_sp, W_sp = 32, self.split_size
        elif idx == 1:
            W_sp, H_sp = 32, self.split_size
        else:
            print ("ERROR MODE", idx)
            exit(0)
        self.H_sp = H_sp
    ...
    def forward(self, qkv,qkv_):
        ...
        _, lepe = self.get_lepe(q, self.get_v)
        q = self.im2cswin(q)
        #print('q:',q.shape)
        #print('k_:',k_.shape)
        k = self.im2cswin(k_)
        ...
        #print('k:',k.shape)
        v, _ = self.get_lepe(v_, self.get_v)
        
        #print('v:',v.shape)

        # if self.sr_ratio > 1:
        #     x_ = qkv.permute(0, 2, 1).reshape(B, C, H, W)
        ...
        v=v.permute(1,2,0,3)
        #print('v p:',v.shape)
        lepe=lepe.permute(1,2,0,3)
        #print('lepe:',lepe.shape)
